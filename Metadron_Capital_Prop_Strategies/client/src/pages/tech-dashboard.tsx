...
const API_SECTIONS: APISection[] = [
  ...
  },
  {
    ...
    auth: "IBKR_HOST + IBKR_PORT + IBKR_CLIENT_ID (TWS/Gateway via ib_insync)",
    status: "configured",
    endpoints: [
      { endpoint: "reqAccountUpdates", purpose: "Account info (equity, cash, buying power)", usedIn: "get_account() / _sync_account()" },
      { endpoint: "reqPositions", purpose: "All open positions", usedIn: "_sync_positions()" },
      { endpoint: "placeOrder / cancelOrder", purpose: "Submit / list / cancel orders", usedIn: "place_order() / cancel_order()" },
      { endpoint: "placeOrder (bracket)", purpose: "Bracket orders (stop-loss + take-profit)", usedIn: "place_order(stop_loss=, take_profit=)" },
      { endpoint: "placeOrder (close)", purpose: "Close specific position", usedIn: "close_position()" },
      { endpoint: "reqGlobalCancel", purpose: "Close all positions", usedIn: "close_all_positions()" },
      { endpoint: "reqPnL / reqPnLSingle", purpose: "Portfolio history for P&L", usedIn: "get_portfolio_history()" },
    ],
  },
  {
