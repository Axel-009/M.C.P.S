...
export default function RiskPortfolio() {
  ...
  const { data: optionsApi } = useEngineQuery<{ positions: OptionsPosition[]; aggregate: Record<string, number> }>("/risk/options-positions", { refetchInterval: 10000 });
  const { data: futuresApi } = useEngineQuery<{ positions: FuturesPosition[]; totals: Record<string, number> }>("/risk/futures-positions", { refetchInterval: 10000 });
